...
const GetStarted = () => {
  ...
  return (
    <motion.div
      ...
      className="w-full -px-40 mx-0 relative text-[#2D2D2D]"
    >
      ...
      <div className="flex flex-col items-center bg- lg:py-10 lg:hidden block py-5">
      ...
      <div className="bg-[#F2F6F9] lg:py-20">
        ...
        <div className="flex flex-wrap justify-center gap-12 py-10">
          {cards.map((item, index) => (
            <Link key={index} to={item.href} className="block"> {/* Use Link from react-router-dom */}
              <Card className="lg:w-72 w-80 pb-16 bg-white cursor-pointer">
                <Image
                  alt="Card background"
                  className="object-cover w-80 lg:w-72 rounded-t-xl"
                  src={item.img}
                />
